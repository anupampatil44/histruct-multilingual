...
class HiStructBert(nn.Module):
    def __init__(self, base_LM, temp_dir, finetune):
        ...
        elif(base_LM=='xlm-roberta-longformer'):
          self.model = HiStructBertModel.from_pretrained('bert-base-multilingual-uncased', cache_dir=temp_dir)
        
        elif(base_LM=='xlm-roberta'):
          self.model = HiStructBertModel.from_pretrained('bert-base-multilingual-uncased', cache_dir=temp_dir)
    ...
    def forward(self, x, segs, mask, tok_struct_vec):
        if(self.finetune):
