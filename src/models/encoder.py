...
class ExtTransformerEncoder(nn.Module):
    def __init__(self, model, args, d_model, d_ff, heads, dropout, num_inter_layers=0):
        ...
        print('init!!! ExtTransformerEncoder')
        self.args = args
        self.d_model = d_model
        self.num_inter_layers = num_inter_layers

        init_logger(self.args.log_file)
        print('encoder.py line 86')
        ...
        if self.args.section_names_embed_path != '':
            print('encoder.py line 126')
            try:
                # print('tym2try')
                # print('torch list:', list(torch.load(self.args.section_names_embed_path).values()))
