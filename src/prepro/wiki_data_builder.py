...
def article_text(doc):
    ...
    for i in doc['sections']:
        for ref in i['references']:
            temp = sentence_tokenize.sentence_split(ref, lang='hi')
            for j in temp:
                doc_list.append(j)
...
def section_text(doc):
    ...
    for i in doc['sections']:
        inside_list=[]
        for ref in i['references']:
            temp_li=sentence_tokenize.sentence_split(ref, lang='hi')
            for j in temp_li:
...
def obtain_histruct_info(doc, args, tokenizer):
    ...
    # src_sent = [i['content'] for i in doc['sections']]
    ...
    #print('len(src_sent):',len(src_sent))
    #print('len(sum(src_para_sent, [])):',len(src_para_sent))
...
class BertData():
    def __init__(self, args):
        ...
        if args.base_LM.startswith('xlm-roberta'):

            self.tokenizer = AutoTokenizer.from_pretrained(args.base_LM)
        elif args.base_LM.startswith('bigbird-pegasus'):

            self.tokenizer = PegasusTokenizer.from_pretrained("google/" + args.base_LM,
                                                              additional_special_tokens=['<unk_2>', '<unk_3>',
                                                                                         '<unk_4>'])
        else:
    ...
    def preprocess(self, doc, src, tgt, sent_labels, is_test=False):
        ...
        skip, skip_reason, section_names, _overall_sent_pos, _sent_struct_vec, _token_struct_vec = obtain_histruct_info(
            doc, self.args, self.tokenizer)
        ...
        src_txt = [sent.replace(self.cls_token, ' '.join([c for c in self.cls_token])).replace(self.sep_token, ' '.join(
            [c for c in self.sep_token])) for sent in src_txt]

        # join sentences into text, add cls_token and sep_token between sentences
        if self.args.base_LM.startswith('xlm-roberta'):
        ...
        tgt_subtokens_str = self.tgt_bos + self.tgt_sent_split.join(
            [' '.join(self.tokenizer.tokenize(tt)) for tt in tgt]) + self.tgt_eos
...
def format_to_histruct(args):
    ...
    if not os.path.exists(args.save_path):
        ...
        if len(os.listdir(args.save_path)) != 0:
            text = input(
                'Save folder already exists and is not empty. Do you want to remove it and redo preprocessing (yes or no) ?')
            if text.lower() == 'yes':
...
def _format_to_histruct(params):
    ...
    if (os.path.exists(save_file)):
        text = input(
            "Save file %s already exists. Do you want to remove it and redo preprocessing (yes or no) ?" % (save_file))
        if text.lower() == 'yes':
            os.remove(save_file)
    ...
    for d in jobs:
        ...
        print(d.keys())
        # print('title',d["title"])
        source, tgt = article_text(d), abstract_text(d)
    ...
    dic = {'file_name': json_file, 'nr. of total doc': len(jobs), 'nr. of processed doc': len(datasets),
           'no. of skipped instances': len(skip_reasons),
           'skip percentage': round(len(skip_reasons) / len(jobs) * 100, 2),
    ...
    logger.info('Skipped instances %d, %f percentage of the %d instances' % (
    len(skip_reasons), round(len(skip_reasons) / len(jobs) * 100, 2), len(jobs)))
...
def merge_data_splits(args):
    ...
    for f in glob.glob(pjoin(args.raw_path, '*.txt')):
        ...
        if data_type == 'val':
            data_type = 'valid'
        data = [json.loads(json.dumps(line)) for line in open(f, 'r', encoding='utf-8')]
        corpora[data_type] = data

    logger.info('There are %s / %s / %s documents in train/valid/test datasets.' % (
    len(corpora['train']), len(corpora['valid']), len(corpora['test'])))
...
def obtain_section_names(args):
    ...
    for f in glob.glob(pjoin(args.raw_path, '*.txt')):
        data = [eval(line) for line in open(f, 'r', encoding='utf-8')]
        for doc in data:
            doc_sec_names = [i['title'] for i in doc['sections']]
...
def encode_section_names(args):
    ...
    if args.base_LM.startswith('xlm-roberta-longformer'):
        model = AutoModel.from_pretrained('markussagen/' + args.base_LM, cache_dir=args.temp_dir)
        model.eval()
        tokenizer = AutoTokenizer.from_pretrained('markussagen/' + args.base_LM)
        ...
        for section_name in section_names:
            ...
            global_attention_mask = torch.ones(input_ids.shape, dtype=torch.long,
                                               device=input_ids.device)  # do global attention everywhere
            ...
            logger.info(
                'section name encoded: %s, (%d/%d) ' % (section_name, len(section_names_embed), len(section_names)))
...
def encode_section_names_cls(args):
    ...
    if args.base_LM.startswith('xlm-roberta-longformer'):
        model = AutoTokenizer.from_pretrained('markussagen/' + args.base_LM, cache_dir=args.temp_dir)
        model.eval()
        tokenizer = AutoTokenizer.from_pretrained('markussagen/' + args.base_LM)
        ...
        for sn in sns:
            if sn not in cls_v:
                ...
                logger.info('section name %s not included in section classes (%d/%d) ' % (
                sn, len(section_names_embed8), len(sns)))
            else:
                for cls in list(sn_cls_dic.keys()):
                    if sn in sn_cls_dic[cls]:
                        ...
                        logger.info('section name %s in section cls %s, (%d/%d) ' % (
                        sn, cls, len(section_names_embed8), len(sns)))
                        break
        # save
        base_lm_name = args.base_LM.split('-')[0] + args.base_LM.split('-')[1][0].upper()
        path8 = args.save_path + '/section_names_embed_' + base_lm_name + '_' + args.sn_embed_comb_mode + 'CLS' + str(
            nr_of_classes) + '.pt'
        path9 = args.save_path + '/section_names_embed_' + base_lm_name + '_' + args.sn_embed_comb_mode + 'CLS' + str(
            nr_of_classes + 1) + '.pt'
        ...
        for sn in sns:
            if sn not in cls_v:
                section_names_embed8.update({sn: sn_emb_dic[sn]})
                section_names_embed9.update({sn: section_cls_embed['others']})
                logger.info('N----section name %s not included in section classes (%d/%d) ' % (
                sn, len(section_names_embed8), len(sns)))
            else:
                for cls in list(sn_cls_dic.keys()):
                    if sn in sn_cls_dic[cls]:
                        section_names_embed8.update({sn: section_cls_embed[cls]})
                        section_names_embed9.update({sn: section_cls_embed[cls]})
                        logger.info('Y----section name %s in section cls %s, (%d/%d) ' % (
                        sn, cls, len(section_names_embed8), len(sns)))
                        break
        # save
        base_lm_name = args.base_LM.split('-')[0] + args.base_LM.split('-')[1][0].upper()
...
def compute_statistics_raw(args):
    ...
    for f in glob.glob(pjoin(args.raw_path, '*.txt')):
        ...
        for doc in data:
            ...
            doc_len_para.append(len(section_text(doc)))
            doc_len_sent.append(len(article_text(doc)))

            summ_len_sent.append(len(abstract_text(doc)))

            src = article_text(doc)
            tgt = [s.replace('<S>', '').replace('</S>', '') for s in abstract_text(doc)]
            flat_src = ''.join(src)
            flat_summ = ''.join(tgt)
...
def get_novel_ngrams_percentage(flat_src, flat_summ, ngrams):
    ...
    flat_summ = flat_summ.split()
    flat_src = flat_src.split()
